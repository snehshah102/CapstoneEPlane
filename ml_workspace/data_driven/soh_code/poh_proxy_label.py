...
def process_plane(cfg: Config) -> dict[str, object]:
    ...
    def _attach_flight_index(df: pd.DataFrame) -> pd.DataFrame:
        if df.empty or "flight_id" not in df.columns:
            return df
        out = df.copy()
        out["flight_index"] = (
            pd.to_numeric(out["flight_id"], errors="coerce").map(flight_index_map).astype("Int64")
        )
        cols = out.columns.tolist()
        cols.remove("flight_index")
        cols.insert(cols.index("flight_id"), "flight_index")
        return out[cols]

    phase_df = _attach_flight_index(phase_df)
    flight_df = _attach_flight_index(flight_df)
    segment_df = _attach_flight_index(segment_df)
    issue_df = _attach_flight_index(issue_df)
    ...
    lookup_path = out_root / "flight_index_lookup.csv"
    ...
    summary["flight_index_lookup_csv"] = str(lookup_path)
    return summary
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("ml_workspace/data_driven/output/poh_proxy"),

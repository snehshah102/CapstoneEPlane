...
From `ml_workspace/soh_forecast/output/multihorizon_runner_plane_166/best_models_by_horizon.csv` for plane 166 validation:

- 1-flight horizon: `random_forest_with_latent`, level MAE 0.0465, level R2 0.9976
- 5-flight horizon: `physics_hybrid_with_latent`, level MAE 0.1605, level R2 0.9801
- 10-flight horizon: `physics_hybrid_with_latent`, level MAE 0.2325, level R2 0.9544
- 15-flight horizon: `gru_sequence_no_latent`, level MAE 0.3041, level R2 0.9353
- 20-flight horizon: `elastic_with_latent`, level MAE 0.2317, level R2 0.9635
...
It is important to state clearly where these results come from. The file `best_models_by_horizon.csv` is not a hand-edited summary and it is not produced by the walkthrough notebook. It is generated by the training pipeline in `ml_workspace/soh_forecast/train_horizon_models.py`.

For each forecast horizon, that pipeline:

1. trains the candidate model family set for the target horizon,
2. computes saved prediction and metric tables,
3. restricts model selection to the validation split,
4. ranks models by `level_mae`, using `delta_mae` as a tiebreaker,
5. writes the winning row to `best_models_by_horizon.csv`,
6. and saves the corresponding model artifact path.

This means the “best model” in the report should be interpreted specifically as:

- the model that won on validation for that horizon,
- not necessarily the model with the best test result,
- and not necessarily the same model family across all horizons.

That distinction matters because the notebook can also display test metrics for the pipeline-selected winner. Those test metrics are for evaluation only. They should not be confused with the validation-based model-selection rule used to create `best_models_by_horizon.csv`.

### 7.6.1 Interpretation of the current horizon winners

The horizon winners tell a useful methodological story:

- At the 1-flight horizon, `random_forest_with_latent` wins. This suggests that the immediate next-flight problem benefits from nonlinear tabular structure and latent-state context.
- At the 5-flight and 10-flight horizons, `physics_hybrid_with_latent` wins. This suggests that over a slightly longer operating window, physically motivated structure plus latent information becomes more useful than a purely tabular local fit.
- At the 15-flight horizon, `gru_sequence_no_latent` wins. This is noteworthy because it implies sequence structure can become more valuable at a medium horizon even without explicit latent inputs, at least under the current benchmark configuration.
- At the 20-flight horizon, `elastic_with_latent` wins. This suggests that at the longest currently supported horizon, a simpler regularized model can outperform more flexible models once the signal becomes smoother and the sample size becomes smaller.

Taken together, these results support an important capstone conclusion: there is no single universally best forecasting model across all decision horizons. The repository is stronger because it allows horizon-specific model selection instead of forcing one model architecture to serve every timescale.

### 7.6.2 Practical interpretation for the deployed workflow

In the final forecasting workflow, the multihorizon results should be read as a stack of operational models:

- the shortest horizon captures immediate operational sensitivity,
- the middle horizons capture accumulated near-term degradation over realistic flight windows,
- and the longest horizon provides a bridge into the long-run backbone trend.

This is also why the notebook and report should stay synchronized to `best_models_by_horizon.csv`. That file is the canonical summary of which saved model should be used for each horizon in downstream visualization, analysis, or frontend integration.
